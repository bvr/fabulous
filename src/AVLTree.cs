// from:
// - https://ericlippert.com/2008/01/18/immutability-in-c-part-eight-even-more-on-binary-trees/
// - https://ericlippert.com/2008/01/21/immutability-in-c-part-nine-academic-plus-my-avl-tree-implementation/
...
public interface IBinarySearchTree<K, V> : IBinaryTree<V>, IMap<K, V>
    where K : IComparable<K>
{
    K Key { get; }
...
public sealed class AVLTree<K, V> : IBinarySearchTree<K, V>
    where K : IComparable<K>
{
    ...
    private static IBinarySearchTree<K, V> DoubleLeft(IBinarySearchTree<K, V> tree)
    {
        if (tree.Right.IsEmpty)
            return tree;
        AVLTree<K, V> rotatedRightChild = new AVLTree<K, V>(
            tree.Key,
            tree.Value,
            tree.Left,
    ...
    private static IBinarySearchTree<K, V> DoubleRight(IBinarySearchTree<K, V> tree)
    {
        if (tree.Left.IsEmpty)
            return tree;
        AVLTree<K, V> rotatedLeftChild = new AVLTree<K, V>(
            tree.Key,
            tree.Value,
